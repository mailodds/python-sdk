...
class BatchDeliverResponse(BaseModel):
    """
    ...
    total: Optional[StrictInt] = Field(default=None, description="Total recipients submitted")
    accepted: Optional[StrictInt] = Field(default=None, description="Number of recipients accepted for delivery")
    rejected: Optional[List[BatchDeliverResponseRejectedInner]] = Field(default=None, description="Recipients that were rejected (suppressed or failed validation)")
    status: Optional[StrictStr] = Field(default=None, description="Batch status")
    delivery: Optional[BatchDeliverResponseDelivery] = None
    __properties: ClassVar[List[str]] = ["schema_version", "request_id", "total", "accepted", "rejected", "status", "delivery"]
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(['queued', 'all_rejected']):
            raise ValueError("must be one of enum values ('queued', 'all_rejected')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "rejected": [BatchDeliverResponseRejectedInner.from_dict(_item) for _item in obj["rejected"]] if obj.get("rejected") is not None else None,
            "status": obj.get("status"),
            "delivery": BatchDeliverResponseDelivery.from_dict(obj["delivery"]) if obj.get("delivery") is not None else None
        })
        return _obj
