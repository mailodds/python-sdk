...
class IdentityScoreCheck(BaseModel):
    """
    IdentityScoreCheck
    """ # noqa: E501
    status: StrictStr = Field(description="Check status (e.g. verified, pending, missing)")
    points: StrictInt = Field(description="Points earned for this check")
    max_points: StrictInt = Field(description="Maximum points available for this check")
    verified_at: Optional[datetime] = Field(default=None, description="When the check was last verified")
    __properties: ClassVar[List[str]] = ["status", "points", "max_points", "verified_at"]
