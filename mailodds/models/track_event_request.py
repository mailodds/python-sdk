...
class TrackEventRequest(BaseModel):
    ...
    TrackEventRequest
    """ # noqa: E501
    event_type: StrictStr = Field(description="Type of commerce event")
    email: StrictStr = Field(description="Email address associated with the event")
    properties: Optional[Dict[str, Any]] = Field(default=None, description="Event-specific data (e.g., order_id, value, product_url)")
    occurred_at: Optional[datetime] = Field(default=None, description="When the event occurred (defaults to now)")
    idempotency_key: Optional[StrictStr] = Field(default=None, description="Unique key to prevent duplicate events (5 min TTL)")
    __properties: ClassVar[List[str]] = ["event_type", "email", "properties", "occurred_at", "idempotency_key"]

    @field_validator('event_type')
    def event_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['purchase', 'cart_abandon', 'browse', 'wishlist', 'review', 'shadow_dns_dwell', 'css_probe', 'identity_stitch']):
            raise ValueError("must be one of enum values ('purchase', 'cart_abandon', 'browse', 'wishlist', 'review', 'shadow_dns_dwell', 'css_probe', 'identity_stitch')")
        return value
